    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            ...
            try
            {
                ...
                _logger.LogInformation($"Attempting login for user: {model.UserId}");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Có thể parse response để lấy thêm thông tin user
                    // var loginResult = JsonSerializer.Deserialize<LoginResponseModel>(responseContent);
                    ...
                    _logger.LogInformation($"User {model.UserId} logged in successfully. Session ID: {HttpContext.Session.Id}");
                ...
                else
                {
                    ...
                    _logger.LogWarning($"Login failed for user {model.UserId}. API response: {response.StatusCode} - {errorContent}");
                    ViewBag.Error = "Tên đăng nhập hoặc mật khẩu không đúng";
                }
            }
