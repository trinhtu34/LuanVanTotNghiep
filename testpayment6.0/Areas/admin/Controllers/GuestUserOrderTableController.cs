...
{
    [Area("admin")]
    public class GuestUserOrderTableController : Controller
    {
        private readonly HttpClient _httpClient;
        private readonly ILogger<GuestUserOrderTableController> _logger;
        private readonly string BASE_API_URL;

        public GuestUserOrderTableController(HttpClient httpClient, ILogger<GuestUserOrderTableController> logger, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> CreateBooking([FromBody] AdminBookingRequest request)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(request.StartingTime))
                {
                    return Json(new { success = false, message = "Vui lòng chọn thời gian bắt đầu" });
                }

                if (!DateTime.TryParse(request.StartingTime, out DateTime parsedStartTime))
                {
                ...
                var tableIds = request.SelectedTables.Select(t => int.Parse(t.TableId)).ToList();
                ...
                var userId = $"ADMIN_{DateTime.Now.Ticks}_{Guid.NewGuid().ToString("N")[..8]}";

                // Calculate totals
                decimal totalFoodPrice = request.SelectedFoods?.Sum(food => food.Price * food.Quantity) ?? 0;
                decimal totalDeposit = request.SelectedTables.Sum(table => table.Deposit);
                ...
                var orderTableRequest = new
                {
                    ...
                    //bookingType = "ADMIN", // Đánh dấu là đặt bàn bởi admin
                    //createdBy = User.Identity?.Name ?? "Admin" // Lưu thông tin admin tạo
                };
                ...
                if (orderTableResponse.IsSuccessStatusCode)
                {
                    ...
                    if (orderTable != null && orderTable.OrderTableId > 0)
                    {
                        ...
                        var successfulTables = new List<string>();
                        ...
                        foreach (var table in request.SelectedTables)
                        {
                            try
                            {
                                ...
                                if (detailResponse.IsSuccessStatusCode)
                                {
                                    successfulTables.Add(table.TableId);
                                }
                                else
                                {
                                    failedTables.Add(table.TableId);
                                }
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, $"Error adding table {table.TableId}");
                                failedTables.Add(table.TableId);
                            }
                        }
                        ...
                        if (request.SelectedFoods != null && request.SelectedFoods.Any())
                        {
                            foreach (var food in request.SelectedFoods)
                            {
                                try
                                {
                                    ...
                                    {
                                        successfulFoods.Add(food.DishName);
                                    }
                                    else
                                    {
                                        failedFoods.Add(food.DishName);
                                    }
                                }
                                catch (Exception ex)
                                {
                        ...
                        // Chuẩn bị response message
                        var message = BuildSuccessMessage(successfulTables, failedTables, successfulFoods, failedFoods);
                        ...
                        if (successfulTables.Any())
                        {
                            return Json(new
                            {
                                ...
                                totalAmount = totalDeposit + totalFoodPrice
                            });
                        }
                        else
                        {
            ...
            {
                _logger.LogError(ex, "Error creating admin booking");
        ...
        public async Task<IActionResult> GetMenuByRegion(int regionId)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (menu == null)
                    {
                        return Json(new List<MenuViewModel>());
                    }
                    return Json(menu);
                }
                else
        ...
        private string BuildSuccessMessage(List<string> successfulTables, List<string> failedTables,
                                 List<string> successfulFoods, List<string> failedFoods)
        {
            var message = $"Đặt bàn thành công cho {successfulTables.Count} bàn: [{string.Join(", ", successfulTables)}]";
